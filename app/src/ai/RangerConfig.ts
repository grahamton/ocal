...
MODE: EXPLORE (Discovery Contract).
You are in Structured Brainstorming Mode. Your goal is to maximize signal discovery while maintaining a consistent structure.

REQUIRED EXPLORE BLOCKS:
1. "ranger_summary": 2-3 short, declarative sentences. The primary user-facing description. Do NOT mention AI.
2. "evidence": Array of short observed visual cues (e.g. "waxy luster", "conchoidal fracture").
3. "discovered_signals": Suggest 5-12 candidate fields that would be valuable for the app schema. For each, provide { "name", "description", "display_recommendation" }.
4. "debug": Freeform notes on your reasoning or uncertainties.

SAFETY RULE:
- Safety content goes ONLY in "caution" or "red_flags". Never embed safety warnings in "historical_fact" or "ranger_summary".`;
  }
...
export const getRangerSchema = (mode: RangerMode = 'explore') => {
  ...
  // Base properties required in all modes (the "Ship" contract)
  ...
  // Properties exclusive to Explore Mode (The "Discovery" contract)
  const exploreProperties = !isStrict ? {
    ranger_summary: { type: 'string', description: '2-3 short declarative sentences summarizing the ID. Never mention AI.' },
    evidence: { type: 'array', items: { type: 'string' }, description: 'Short observed visual cues (e.g. "waxy luster", "banding").' },
    context_hypotheses: { type: 'array', items: { type: 'string' }, description: 'Plausible stories about formation or transport.' },
    discovered_signals: {
      type: 'array',
      description: 'Candidate fields that could be useful for the future app schema.',
      items: {
        type: 'object',
        properties: {
          name: { type: 'string' },
          description: { type: 'string' },
          display_recommendation: { type: 'string', enum: ['default', 'context', 'user', 'hidden'] }
        }
      }
    },
    debug: { type: 'object', description: 'Freeform notes, ghost fields, and heuristics for developer analysis.' }
  } : {};
  ...
  return {
    ...
    schema: {
      ...
      required: [
        ...
        'specimen_context',
        'lapidary_guidance',
        // In Explore mode, we want these to populate, but strict=false means we can't force them hard.
        // However, listing them here helps the model know they are expected.
        ...(isStrict ? [] : ['ranger_summary', 'evidence', 'discovered_signals'])
      ],
      properties: {
        ...baseProperties,
        ...exploreProperties
