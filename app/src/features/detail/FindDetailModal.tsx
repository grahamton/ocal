...
export function FindDetailModal({visible, item, onClose, onSaved}: Props) {
  ...
  const handleDelete = () => {
    Alert.alert('Delete this find?', 'This cannot be undone.', [
  ...
  return (
    <Modal
      ...
      <View style={[styles.container, {backgroundColor: colors.background}]}>
        ...
        <TouchableOpacity
          style={[styles.closeBtn, {zIndex: 50}]}
          onPress={onClose}>
            ...
            <>
              ...
              <View
                ...
                <View style={styles.labHeader}>
                  <StatusIcon
                    status={
                      aiResult ? 'polished' : aiLoading ? 'polishing' : 'rough'
                    }
                ...
                {aiResult && (
                  <>
                        ...
                        {aiResult.category_details.rock && (
                          <>
                            ...
                            <ContextItem
                              label="Texture"
                              value={
                                aiResult.category_details.rock.texture_type ||
                                ''
                              }
                              colors={colors}
                            />
                        ...
                        {aiResult.category_details.fossil && (
                          <>
                            ...
                            <ContextItem
                              label="Taxonomy"
                              value={
                                aiResult.category_details.fossil.taxonomy || ''
                              }
                              colors={colors}
                            />
                            <ContextItem
                              label="Living Relative"
                              value={
                                aiResult.category_details.fossil
                                  .living_relative || ''
                              }
                    ...
                    {((aiResult.red_flags && aiResult.red_flags.length > 0) ||
                      (aiResult.caution && aiResult.caution.length > 0)) && (
                      <View
                        style={[
                          styles.alertBox,
                          {backgroundColor: '#fef2f2', borderColor: '#ef4444'},
                        ...
                        <View style={{flex: 1}}>
                          {[
                            ...
                            <Text
                              key={i}
                              style={[styles.alertText, {color: '#b91c1c'}]}>
                              • {flag}
                            </Text>
                          ))}
                        </View>
                    ...
                    {aiResult.lapidary_guidance && (
                      <View style={styles.labSection}>
                        <View style={styles.rowCenter}>
                          <Ionicons
                            name={
                              aiResult.lapidary_guidance.is_tumble_candidate
                                ? 'checkmark-circle'
                                : 'close-circle'
                            }
                            size={18}
                          ...
                          <Text
                            ...
                            {aiResult.lapidary_guidance.is_tumble_candidate
                              ? 'Good for Tumbling'
                              : 'Not Recommended for Tumbling'}
                          </Text>
                        </View>
                        <Text
                          style={[
                            styles.bodyText,
                            {color: colors.textSecondary, marginLeft: 26},
                          ]}>
                    ...
                    {aiResult.region_fit &&
                      (aiResult.region_fit.fit === 'medium' ||
                        aiResult.region_fit.fit === 'low') && (
                        <View style={styles.labSection}>
                          <View style={styles.rowCenter}>
                            <Ionicons
                            ...
                            <Text
                              ...
                              Location Check:{' '}
                              {aiResult.region_fit.fit.toUpperCase()}
                            </Text>
                          </View>
                          <Text
                            style={[
                    ...
                    {((aiResult.best_guess?.confidence || 0) < 0.9 ||
                      aiResult.best_guess?.category === 'unknown') &&
                      aiResult.followup_photos &&
                      aiResult.followup_photos.length > 0 && (
                        <View style={styles.labSection}>
                          <Text
                            style={[
                              styles.label,
                            ...
                            Ranger Tip: Try these photos for better ID
                          </Text>
                          <View
                            style={{
                              flexDirection: 'row',
                    ...
                    {showContext && aiResult.specimen_context && (
                      <View style={styles.toggleContent}>
                        <ContextItem
                          label="Age"
                          value={aiResult.specimen_context.age}
                          colors={colors}
                        />
                        <ContextItem
                        ...
                        <ContextItem
                          ...
                          value={aiResult.specimen_context.historical_fact}
                          colors={colors}
                        />
                      </View>
                    )}
                        ...
                        {aiResult.alternatives &&
                          aiResult.alternatives.length > 0 && (
                            <View style={{marginBottom: 12}}>
                              ...
                              {aiResult.alternatives.map(
                                (
                                  alt: {label: string; confidence: number},
                                  i: number,
                                ) => (
                        ...
                        {aiResult.catalog_tags && (
                          <View>
                            ...
                            <Text style={{color: colors.text, lineHeight: 20}}>
                              {Object.entries(aiResult.catalog_tags)
                                .map(([_k, v]) =>
