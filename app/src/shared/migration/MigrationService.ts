    ...
    try {
      ...
      for (const find of finds) {
        try {
          ...
          logger.error('MigrationService: Failed to migrate find', {
            findId: find.id,
            error,
          });
      ...
      for (const session of sessions) {
        try {
          ...
          logger.error('MigrationService: Failed to migrate session', {
      ...
      logger.add(
        'migration',
        `Data migration complete. Migrated ${completedItems} items.`,
      );
    } catch (error) {
      const errorMessage =
