...
const { ROCK_ID_SYSTEM_PROMPT, buildRockIdUserPrompt } = require('./rockIdPrompt');

const ajv = new Ajv({ allErrors: true, strict: false });
const validate = ajv.compile(RockIdSchema.schema);

function clampImages(body) {
  const urls = Array.isArray(body.image_urls) ? body.image_urls.filter(Boolean) : [];
  const dataUrls = Array.isArray(body.image_data_urls) ? body.image_data_urls.filter(Boolean) : [];
...
async function callOpenAI({ userPrompt, images }) {
  ...
  const resp = await client.responses
    .create({
      model: 'gpt-4.1-mini',
      input: [
        { role: 'system', content: [{ type: 'input_text', text: ROCK_ID_SYSTEM_PROMPT }] },
        { role: 'user', content },
      ],
      text: {
        format: {
...
function toGeminiParts({ userPrompt, images }) {
  ...
  parts.push({ text: 'Return JSON only matching the provided schema. Keep confidence monotonic.' });
  return parts;
}
...
exports.identify = onRequest(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!parsed) {
        return res.status(500).json({ error: 'Response was not valid JSON', detail: raw?.slice(0, 400) ?? 'empty' });
      }

      const valid = validate(parsed);
      if (!valid) {
        return res.status(500).json({ error: 'Schema validation failed', detail: validate.errors });
      }
